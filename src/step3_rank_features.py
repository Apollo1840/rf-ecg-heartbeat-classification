...
def construct_vectors(beats):
    ...
    features_names += ["wt_coef_" + str(i) for i, x in enumerate(beats[0]["wt"])]  # why not use range(len())
    ...
    labels = np.array([beat["beatType"].value for beat in beats], dtype=int)
    sources = [beat["source"] for beat in beats]
...
if __name__ == "__main__":
    ...
    S_ind = np.array([k for k, l in enumerate(labels_L) if l == BeatType.AURICULAR_PREMATURE_CONTRACTION.value],
                     dtype=int)
    V_ind = np.array([k for k, l in enumerate(labels_L) if l == BeatType.PREMATURE_VENTRICULAR_CONTRACTION.value],
                     dtype=int)
    indixes = np.block([N_ind, S_ind, V_ind])  # equal to: indixes = np.concatenate((N_ind, S_ind, V_ind), axis=0)

    features_SV = train_features[indixes]  # np.array
    labels_SV = train_labels[indixes]   # List[int]
